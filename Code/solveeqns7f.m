...
(*
Print["number of detectors: ",detectnums];
*)
If[Length[pts]!=4,Print["this function expects to be given 4 points"];Return[]];
(*Print["check4 at : ",answ]; *)
(*
If[Length[answ[[1,1]] ]<6 || Length[answ[[2,1]] ]<6 ||Length[answ[[3,1]] ]<6 ||Length[answ[[4,1]] ]<6,Print["not able to check4pts at ", pts,"  ",answ];Return[{}]];
*)
If[answ =={} || Length[answ]<4 ,Print["not able to check4pts at ", pts,"  answ0"];Return[{}]];
If[answ[[1]] =={} || answ[[2]]=={} ||answ[[3]] =={} ||answ[[4]] =={},Print["not able to check4pts at ", pts,"  answ"];Return[{}]];
(*
If[Length[answ]>1,If[Length[answ[[2,1]] ]<4 || Length[answ[[2,2]] ]<4 ||Length[answ[[2,3]] ]<4 ||Length[answ[[2,4]] ]<4,Print["not able to check4pts, length >1"];Return[{}]]];
*)
...
Print[Length[detectors[[1]]], " detectors"];
*)
  detectlist = {};
...
For[kk=1,kk<=Length[ttmmpp],++kk,
  ...
  For[j=1,j<=Length[pts],++j,
   If[printflag>0,Print["j =  ",j]]; 
	detect[j]=(detectors[[j]]/.ttmmpp[[kk,2,j]]);
   If[printflag>0,Print["this detector: ",detect[j]]];
];
  For[j = 1, j <= detectnums, ++j,
   For[k = j + 1, k <= detectnums, ++k,
    testpts = {j, k};  (*Print[{j,k}]; *)
...
If[ttmmpp[[1,2,1]] == {} || ttmmpp[[1,2,2]] == {} ||ttmmpp[[1,2,3]] == {} ||ttmmpp[[1,2,4]] =={},Print["not able to check4pts ttmmpp"];Return[{}]];
  detectlist = {};
dim=Length[pts[[1]]];
If[printflag>0,Print["Doing a ",dim, " dimensional search."]];
For[kk=1,kk<=Length[ttmmpp],++kk,
  subdetectlist={};
  For[j=1,j<=Length[pts],++j,
   Print["j =  ",j]; 
...
(* need to modify the output of this function *)
searchgrid2[ll_, ur_, numx_, numy_, numterms_, FE_, gtab_, stab_, detectg_, detects_,Ev_,gflag_,PRECIS_,absflag_]:=
searchgrid2[ll, ur, numx, numy,numterms, FE, gtab, stab, detectg, detects, Ev,{},gflag,PRECIS,absflag];

searchgrid2[ll_, ur_, numx_, numy_, numtermsIN_, FEin_, gtabIN_, stabIN_, detectg_, detects_,EvIN_,coeffstart_,gflag_,PRECIS_,absflag_]:= searchgrid2[ll, ur, numx, numy, numtermsIN, FEin,
	gtabIN, stabIN, detectg, detects,EvIN,coeffstart,gflag,PRECIS,absflag,{{3,{1}},{{},{}}}];

searchgrid2[ll_, ur_, numx_, numy_, numtermsIN_, FEin_, gtabIN_, stabIN_, detectg_, detects_,EvIN_,coeffstart_,gflag_,PRECIS_,absflag_,EP_]:=Block[{jz,kz,numterms,FE,numtermsTAB,Ev},
DETECTPRECIS=PRECIS;
detectedpoints={};
hitpoints={};
closepoints={};
...
FE=FEin;
FE[[2]]=pairtotriple[ur];
lev=(FE[[-2]]*Pi^(3/2))^2;
    nu=EvIN[[1]];
    istep=stepsizeRM[nu, PRECIS];
    ev={nu,istep};

Print["testing ",N[ll, PRECIS], " to ",N[ur,PRECIS], " level ",lev];
  xstep = (ur[[1]] - ll[[1]])/numx;
  ystep = (ur[[2]] - ll[[2]])/numy;
  xvec={xstep,0};
  yvec={0,ystep};
  numterms=If[numtermsIN>1,numtermsIN,
	numtermsTAB=  (* this is wrong, because it assumes gtabIN and stabIN are sorted *)
	Table[{findSumlim[FE, gtabIN[[j,1]], stabIN[[j]], ev, Abs[numtermsIN],gflag,PRECIS,Sign[absflag]][[1]],
	findSumlim[FE, gtabIN[[j,2]], stabIN[[j]], ev, Abs[numtermsIN],gflag,PRECIS,Sign[absflag]][[1]],
	findSumlim[FE, gtabIN[[-j,1]], stabIN[[-j]], ev,
Abs[numtermsIN],gflag,PRECIS,Sign[absflag]][[1]],
        findSumlim[FE, gtabIN[[-j,2]], stabIN[[-j]], ev,
Abs[numtermsIN],gflag,PRECIS,Sign[absflag]][[1]]},
	{j,1,2}];  (* note:  we only check the first and last equations *)
	Max[numtermsTAB]];
Print["Number of terms in sum: ",numterms];
  Ev={nu,numterms};
  unknowns=theunknowns[EP,numterms];
  If[Length[unknowns]>Length[gtabIN],Print["Not enough gtabIN functions"];
		Return[]];
  If[Length[unknowns]>Length[stabIN],Print["Not enough stabIN points"];
		Return[]];
  gtab=Take[gtabIN,Length[unknowns]];
  stab=Take[stabIN,Length[unknowns]];
  While[Length[unknowns]>Length[startcoeffs],
     AppendTo[startcoeffs,0]];  (* pad starting values with 0 if necessary*)
  zerostart= Table[{unknowns[[jz]],startcoeffs[[jz]]},{jz,1,Length[unknowns]}];
  Print["Searching with starting values: ",zerostart];

  thept=ll;

  eq1 = makeequations[FE, pairtotriple[thept], gtab, stab, Ev,gflag,PRECIS];
  eq1solv = converteqnsALL[EP, eq1, numterms];

Print["made first set of equations"];

  ans[thept]= findsolmult[eq1solv, unknowns, {zerostart},250,0.001,{4,0.1}];
  ans[thept]=Sort[ans[thept]];
Print[ans[thept]];

Print["solved first set of equations"];

detecteq[thept]=makeequations[FE, pairtotriple[thept], detectg, detects, Ev,gflag,DETECTPRECIS];
(*detecteq[thept]=converteqnsALL[EP, detecteq[thept],numterms];*)
detecteq[thept]=converteqnsALL[EP, detecteq[thept],numterms,1];

Print["made first set of detectors"];

  If[Length[ans[ll]]>0,startvals=Append[substopairs[ans[ll]],zerostart],
        startvals={zerostart}];

  thept=ll+xvec;

  eq = makeequations[FE, pairtotriple[thept], gtab, stab, Ev,gflag,PRECIS];
  eqsolv = converteqnsALL[EP, eq, numterms];

  ans[thept]= findsolmult[eqsolv, unknowns, startvals,NumPtsToTest[startvals],0.001,{4,0.1}];
  ans[thept]=Sort[ans[thept]];
Print[ans[thept]];
...
Print["about to do 4th point"];

  If[Length[ans[ll]]>0 && Length[ans[ll+xvec]]>0 && Length[ans[ll+yvec]]>0,
Print["making startvals "];
        startvals=multipleinterppairs[{ll,ll+xvec,ll+yvec},
                {ans[ll],ans[ll+xvec],ans[ll+yvec]},
                ll+xvec+yvec,4], (*else*)
        startvals=substopairs[ans[ll+xvec]]];
  AppendTo[startvals,zerostart];
  thept=ll+xvec+yvec;
Print["made thept ",thept];
  eq = makeequations[FE, pairtotriple[thept], gtab, stab, Ev,gflag,PRECIS];
  eqsolv = converteqnsALL[EP, eq, numterms];
  ans[thept]= findsolmult[eqsolv, unknowns, startvals,NumPtsToTest[startvals],0.001,{4,0.1}];
  ans[thept]=Sort[ans[thept]];
...
If[ans[ll]=={} || ans[ll+xvec]=={} || ans[ll+yvec]=={} || ans[ll+xvec+yvec]=={},
        Print["Can't get started:  one of the first 4 points has no solution"];
        Return[{}]];
ourpts={ll,ll+xvec,ll+yvec};
detectpts=Append[ourpts,thept];
detectans=Table[ans[detectpts[[jp]]],{jp,1,Length[detectpts]}];
thedetectors=Table[detecteq[detectpts[[jp]]],{jp,1,Length[detectpts]}];
(*
Print["about to check4pts", detectpts];
*)
jjnk1a=check4pts[detectpts,detectans,thedetectors];

Print[Coefficient[jjnk1a,y]];

jjnk1 = (jjnk1a/.{y->0});

For[kk=1,kk<=Length[jjnk1],++kk,
AppendTo[detectedpoints,{detectpts,jjnk1[[kk]]}];

If[insquare[{detectpts,jjnk1[[kk]]}],themedian=Median[jjnk1[[kk]]];
	cp=closepts[{xstep,ystep},jjnk1[[kk]]];
	AppendTo[hitpoints,{themedian,multipleinterppairs[detectpts,detectans,themedian,4]}];
        Print["HIT:  ",Length[detectedpoints],"   ",cp,"   ",themedian,"   ",detectpts,"   ",jjnk1[[kk]],"  ",hitpoints[[-1]]]];

If[(cp=closepts[{xstep,ystep},jjnk1[[kk]]])>0,themedian=Median[jjnk1[[kk]]];
        AppendTo[closepoints,{themedian,cp,multipleinterppairs[detectpts,detectans,themedian,4]}];
        Print["CLOSENESS:  ",Length[detectedpoints],"   ",themedian,"   ",cp,"    ",detectpts,"   ",jjnk1[[kk]]," ",closepoints[[-1]]]]];


If[Max[numx,numy]==1, 
Return[{themedian,cp,ur-ll,numtermsIN,PRECIS,multipleinterppairs[detectpts,detectans,themedian,4],absflag}]
];


Print["about to do first two columns"];
  startvals={zerostart};
  For[jz=2,jz<=numy,++jz,
        thept=ll + jz yvec;
        ourpts={thept-yvec,thept-yvec+xvec,thept-2 yvec};
        startvals=multipleinterppairs[ourpts,
                        Table[ans[ourpts[[j]]],{j,1,Length[ourpts]}], thept,4];
        Print["Testing at ", thept, " with ", startvals];
        If[startvals=={},startvals=substopairs[ans[ourpts[[1]]]]];
        AppendTo[startvals,zerostart];
        eq = makeequations[FE, pairtotriple[thept], gtab, stab, Ev,gflag,PRECIS];
        eqsolv = converteqnsALL[EP, eq, numterms];
        ans[thept]= findsolmult[eqsolv, unknowns, startvals,NumPtsToTest[startvals],0.001,{4,0.1}];
        ans[thept]=Sort[ans[thept]];
  If[Length[detecteq[thept]]<2,
       detecteq[thept]=makeequations[FE, pairtotriple[thept], detectg, detects, Ev,gflag,DETECTPRECIS]; detecteq[thept]=converteqnsALL[EP, detecteq[thept],numterms]];

        thept=ll + jz yvec + xvec;
        ourpts={thept-yvec,thept-xvec, thept-yvec-xvec};
        startvals=multipleinterppairs[ourpts,
                        Table[ans[ourpts[[j]]],{j,1,Length[ourpts]}], thept,4];
        Print["Testing at ", thept, " with ", startvals];
        If[startvals=={},startvals=substopairs[ans[ourpts[[1]]]]];
...
Print["about to check4pts", detectpts];
*)
jjnk1=check4pts[detectpts,detectans,thedetectors];
...
If[insquare[{detectpts,jjnk1[[kk]]}],themedian=Median[jjnk1[[kk]]];
        AppendTo[hitpoints,{themedian,multipleinterppairs[detectpts,detectans,themedian,4]}];
        Print["HIT:  ",Length[detectedpoints],"   ",detectpts,"   ",themedian,"   ",jjnk1[[kk]],"  ",hitpoints[[-1]]]];

If[(cp=closepts[{xstep,ystep},jjnk1[[kk]]])<1/2,themedian=Median[jjnk1[[kk]]];
        AppendTo[closepoints,{themedian,cp,multipleinterppairs[detectpts,detectans,themedian,4]}];
        Print["CLOSE:  ",Length[detectedpoints],"   ",themedian,"   ",cp,"    ",detectpts,"   ",jjnk1[[kk]]," ",closepoints[[-1]]]   ]];
  ];

Print["about to do the rest"];
  For[kz=2,kz<=numx,++kz,
    For[jz=0, jz<=numy,++jz,
(*
Print["jz =",jz];
*)
        thept=ll + kz xvec + jz yvec;
detecteq[thept-2 xvec]={};
        If[jz==0,ourpts={thept-xvec,thept-xvec+yvec,thept-2 xvec},
                ourpts={thept-yvec,thept-xvec-yvec,thept- xvec}];
        pastvals=Table[ans[ourpts[[j]]],{j,1,Length[ourpts]}];
        startvals=multipleinterppairs[ourpts,pastvals,thept,4];
        If[startvals=={},
                j11=1; While[Length[ans[ourpts[[j11]]] ]==0,++j11];
                If[j11<=3,startvals=substopairs[ ans[ourpts[[j11]]] ] ]
           ];
        AppendTo[startvals,zerostart];
        eq = makeequations[FE, pairtotriple[thept], gtab, stab, Ev,gflag,PRECIS];
        eqsolv = converteqnsALL[EP, eq, numterms];
...
thedetectors=Table[detecteq[detectpts[[jp]]],{jp,1,Length[detectpts]}];
(*
Print[jz,"about to check4pts ",detectpts];
*)
jjnk1=check4pts[detectpts,detectans,thedetectors];
For[kk=1,kk<=Length[jjnk1],++kk,
...
{hitpoints,closepoints}
];

zoom1[eigguessIN_, eigDeltaIN_, numtermsIN_, FEin_, gtabIN_, stabIN_, detectg_, detects_,EvIN_,coeffstartIN_,gflag_,PRECISin_,absflag_]:= zoom1[eigguessIN, eigDeltaIN, numtermsIN, FEin,
	gtabIN, stabIN, detectg, detects,EvIN,coeffstartIN,gflag,PRECISin,absflag,{{3,{1}},{{},{}}}];

zoom1[eigguessIN_, eigDeltaIN_, numtermsIN_, FEin_, gtabIN_, stabIN_, detectg_, detects_,EvIN_,coeffstartIN_,gflag_,PRECISin_,absflag_,EP_]:=Block[{},
eigguess=eigguessIN;
eigDelta=eigDeltaIN;
eigguess=eigDelta/100 Floor[eigguess 100/eigDelta];  (* make eigguess exact *)

numterms=numtermsIN;
coeffstart=coeffstartIN;
PRECIS=PRECISin;

Print["PRECIS: ",PRECIS];
Print["eigDelta: ",eigDelta];
count=1;
ll=eigguess-eigDelta/2;
ur=eigguess+eigDelta/2;

Print[Date[]];
Print[MemoryInUse[]];

curroutput=searchgrid2[ll, ur, 1, 1, numterms, FEin, gtabIN, stabIN, detectg, detects,EvIN,coeffstart,gflag,PRECIS,absflag,EP];

If[Length[curroutput]<3,Return["curroutput is : ", curroutput]];

While[count<5,++count;
Print[Date[]];
Print[MemoryInUse[]];
  If[Length[curroutput]<3,Return["curroutput is : ", curroutput]];
  currprecision=Precision[curroutput[[-2]]];
  If[currprecision <2 || Precision[curroutput[[1]]]==Infinity, Print["re-running with more precision: old precision: ", PRECIS];
   Print["current output: ",curroutput];
   Print["which has precision: ",currprecision];
   PRECIS+=10; ClearGam[],
    eigguess=curroutput[[1]];
    closeness=curroutput[[2]];
    closenessfactor=Ceiling[1000 closeness]/1000;
    Print["{closeness, closenessfactor} = ",{closeness,closenessfactor}];
    If[!NumberQ[closenessfactor] || !NumberQ[eigguess[[1]]],
	Print["Error:  not a number, closeness = ",closeness, "  and eigguess = ",eigguess];
	Print["curroutput : ",curroutput];
	Return[]];
    eigDelta *= closenessfactor;  (*zoom in by closeness, rounded *)
    eigguess=eigDelta/100 Floor[eigguess 100/eigDelta];
    numterms *= closenessfactor;  (*zoom in by closeness, rounded *)
    coeffstart = (Transpose[curroutput[[-2,1]]][[-1]]);
    If[currprecision<12, If[currprecision<7,PRECIS+=10,PRECIS+=5]; (* 2<p<7:  +10; 7<p<12: +5 *)
	Print["output precision was ",currprecision,
		" so increased working precision to ", PRECIS];
		ClearGam[]];
    Print["zooming with {eigguess,eigDelta,numterms,PRECIS} = ",
	  {eigguess,eigDelta,numterms,PRECIS}];
    Print["and coefficients : ",coeffstart];
    Print["which have precision : ",currprecision];
    ll=eigguess-eigDelta/2;
    ur=eigguess+eigDelta/2;
     ]; (* if *)
    curroutput=searchgrid2[ll, ur, 1, 1, numterms, FEin, gtabIN, stabIN, detectg, detects,EvIN,coeffstart,gflag,PRECIS,absflag,EP];
...
zoom1[x_,FEin_,gtabIN_, stabIN_, detectg_, detects_,EvIN_]:=
	zoom1[x[[1]], x[[3]] Ceiling[1000 x[[2]] ]/1000, x[[4]], 
		FEin, gtabIN, stabIN, detectg, detects, EvIN, Transpose[x[[6,1]]][[2]], 
		1 (* setting gflag=1, needs to be a parameter *),
		x[[5]] + Max[0, Floor[Precision[12 - x[[6]]]]], x[[7]]];
